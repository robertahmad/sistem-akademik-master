...
export default async function VerifyUkkPage({ params }) {
  const { examId } = params;
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", justifyContent: "center", alignItems: "center", background: "#f8fafc", padding: "1rem" }}>
         <div style={{ background: "white", padding: "2rem", borderRadius: "12px", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.1)", textAlign: "center", maxWidth: "400px" }}>
            ...
            <p style={{ color: "#64748b" }}>Data sertifikat dengan nomor referensi ini tidak ditemukan atau tidak valid.</p>
         </div>
      </div>
    );
  }
           ...
           <table style={{ width: "100%", borderCollapse: "collapse" }}>
             <tbody>
               <tr>
                 ...
                 <td style={{ padding: "0.5rem 0", fontWeight: "bold", color: "#1e293b" }}>{exam.student.name}</td>
               </tr>
               <tr>
                 <td style={{ padding: "0.5rem 0", color: "#64748b", fontSize: "0.9rem" }}>NISN</td>
                 <td style={{ padding: "0.5rem 0", fontWeight: "bold", color: "#1e293b" }}>{exam.student.nisn}</td>
               </tr>
               <tr>
                 <td style={{ padding: "0.5rem 0", color: "#64748b", fontSize: "0.9rem" }}>Skema UKK</td>
                 <td style={{ padding: "0.5rem 0", fontWeight: "bold", color: "#1e293b" }}>{exam.scheme.title}</td>
               </tr>
               <tr>
                 <td style={{ padding: "0.5rem 0", color: "#64748b", fontSize: "0.9rem" }}>Asesor Penguji</td>
                 <td style={{ padding: "0.5rem 0", fontWeight: "bold", color: "#1e293b" }}>{exam.assessor?.name || "Asesor Internal"}</td>
               </tr>
               <tr style={{ borderTop: "1px solid #cbd5e1" }}>
                 <td style={{ padding: "1rem 0 0 0", color: "#64748b", fontSize: "0.9rem" }}>Hasil / Predikat</td>
                 <td style={{ padding: "1rem 0 0 0", fontWeight: "bold", color: isLulus ? "#16a34a" : "#dc2626", fontSize: "1.2rem" }}>{exam.predikat}</td>
               </tr>
             </tbody>
           </table>
        </div>
